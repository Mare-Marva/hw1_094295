...
def preprocess_test(data):
    ...
    for indx, row in data.iterrows():
        ...
        # data_list.append(row['season_released'])

        ## one hot encoders

        ## genres encode
        genres = columns[27:46]
    ...
    df.to_csv('fix_test2.csv', header=True, index=False)
    return df
...
if __name__=='__main__':
    ...
    test_data = pd.read_csv('fix_test2.csv')
    test_data['budget'] = np.log(test_data['budget'].astype('float64'))
    test_data['vote_count'] = np.log(test_data['vote_count'].astype('float64'))

    X = test_data
    X.columns = range(0, 125)

    #####
    model = pickle.load(open("model_lgb1.pickle.dat", "rb"))
    y_pred = model.predict(X)
    y_pred = np.exp(y_pred)
    print("RMSLE is: {:.6f}".format(rmsle(np.exp(y), y_pred)))
